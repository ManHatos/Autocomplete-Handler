...
module.exports = {
	user: async function user(username) {
		...
		if (parseInt(username).isNaN) {
			let userid = await axios.post('https://users.roblox.com/v1/usernames/users', {
				usernames: [username],
			...
			let userAPI = await axios.get('https://users.roblox.com/v1/users/${userid}');
			userAPI.data.avatar =
				'https://cdn.discordapp.com/attachments/893226654086864966/968269659348562011/unknown.png';
			let avatarAPI = await axios.get(`https://thumbnails.roblox.com/v1/users/avatar?userIds=${userid}&size=720x720&format=Png&isCircular=false`);
