...
client.on("interactionCreate", async (interaction) => {
  if (interaction.type === 2) {
    ...
    console.log(
      `${interaction.user.tag} (#${interaction.user.id}) is using '${interaction.options.getFocused(true).name}' Autocomplete on /${interaction.commandName}:   ${interaction.options.getFocused(true).name}`
    );
  }
